...
public class S3CleanupService {
    ...
    public void cleanupUnusedFiles() {
        try {
            ...
            int deletedCount = deleteFilesNotInDatabase();
            
            if (deletedCount > 0) {
                log.info("✅ S3 자동 정리 완료: {}개 파일 삭제됨", deletedCount);
            } else {
    ...
    /**
     * 수동 정리 - 관리자가 필요시 호출 가능
     */
    public int manualCleanup() {
        log.info("🔧 수동 S3 정리 시작 - DB에 없는 파일들");
        return deleteFilesNotInDatabase();
    }
}
