...
public class CampaignMissionInfo {
    ...
    private String[] titleKeywords;  // 제목에 사용할 키워드 목록 (상시 캠페인일 경우 null 허용)
    ...
    /**
     * 상시 캠페인용 미션 정보 유효성을 검증합니다.
     * 상시 캠페인의 경우 titleKeywords가 null일 수 있습니다.
     * @param isAlwaysOpen 상시 캠페인 여부
     * @return 유효성 여부
     */
    public boolean isValidForAlwaysOpenCampaign(boolean isAlwaysOpen) {
        if (!isAlwaysOpen) {
            return isValidMission(); // 일반 캠페인은 기존 검증 로직 사용
        }
        
        // 상시 캠페인의 경우 titleKeywords가 null이어도 허용
        return missionStartDate != null && 
               missionDeadlineDate != null && 
               !missionStartDate.isAfter(missionDeadlineDate);
    }
    ...
    /**
     * 상시 캠페인에서 필수 키워드가 있는지 확인합니다.
     * 상시 캠페인의 경우 titleKeywords는 선택사항입니다.
     * @param isAlwaysOpen 상시 캠페인 여부
     * @return 필수 키워드 존재 여부
     */
    public boolean hasRequiredKeywords(boolean isAlwaysOpen) {
        if (!isAlwaysOpen) {
            return hasKeywords(); // 일반 캠페인은 모든 키워드 필요
        }
        
        // 상시 캠페인의 경우 bodyKeywords만 있어도 됨
        return (bodyKeywords != null && bodyKeywords.length > 0);
    }
}
